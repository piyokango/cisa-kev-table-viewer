...
function normalizeRow(v) {
  ...
  return {
    ...
    requiredAction: translateRequiredAction(v.requiredAction || ""),
    requiredActionOriginal: v.requiredAction || "",
    requiredActionCategory: categorizeRequiredAction(v.requiredAction || "").category,
    dueDate: v.dueDate || "",
...
function categorizeRequiredAction(action) {
  ...
  const standardRules = [
    {
      category: "緩和策適用または使用中止",
      test: /apply mitigations per vendor instructions or discontinue use of (?:the )?product if mitigations are unavailable/i,
      text: "ベンダの案内に従って緩和策を適用してください。緩和策が利用できない場合は、当該製品の使用を中止してください。",
    },
    {
      category: "更新または使用中止",
      test: /apply updates per vendor instructions or discontinue use of (?:the )?product if updates are unavailable/i,
      text: "ベンダの案内に従って更新プログラムを適用してください。更新が利用できない場合は、当該製品の使用を中止してください。",
    },
    {
      category: "更新適用",
      test: /apply updates per vendor instructions/i,
      text: "ベンダの案内に従って更新プログラムを適用してください。",
    },
    {
      category: "緩和策適用",
      test: /apply mitigations per vendor instructions/i,
      text: "ベンダの案内に従って緩和策を適用してください。",
    },
    {
      category: "使用中止",
      test: /discontinue use of (?:the )?product/i,
      text: "当該製品の使用を中止してください。",
    },
    {
      category: "EOL製品の隔離・使用中止",
      test: /(end-of-life|end of life|eol).*(disconnect|discontinue|remove)|(?:disconnect|discontinue|remove).*(end-of-life|end of life|eol)/i,
      text: "サポート終了製品として扱い、ネットワークから切り離すか使用を中止してください。",
    },
    {
      category: "ネットワークから切り離し",
      test: /(disconnect|remove).*(network|internet)|network.*(disconnect|remove)/i,
      text: "当該製品またはシステムをネットワークから切り離してください。",
    },
    {
      category: "指定バージョンへ更新",
      test: /(?:upgrade|update) to .+/i,
      text: `指定されたバージョンへ更新してください。対象の記載: ${normalized}`,
    },
  ];

  const matched = standardRules.find((rule) => rule.test.test(normalized));
  if (matched) {
    return {
  ...
  if (lower.includes("vendor") && (lower.includes("instruction") || lower.includes("guidance"))) {
    return {
      category: "ベンダ案内に従う",
      text: "ベンダの案内に従って必要な対応を実施してください。",
      isStandard: true,
    };
  }
  ...
  return {
    category: "個別対応",
    text: `個別対応が必要です。原文を確認してください: ${normalized}`,
    isStandard: false,
  };
}

function translateRequiredAction(action) {
  return categorizeRequiredAction(action).text;
}
...
function createRequiredActionCell(row) {
  ...
  category.className = "actionCategory";
  category.textContent = row.requiredActionCategory || "個別対応";
