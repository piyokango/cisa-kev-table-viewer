...
function categorizeRequiredAction(action) {
  ...
  const hasVendorInstruction = /vendor instructions?|vendor guidance|vendor recommendations?|vendor directions?/i.test(normalized);
  const hasApplicableGuidance = /applicable .*guidance|in accordance with .*guidance|follow .*guidance/i.test(normalized);
  ...
  if (/bod\s*22-01|binding operational directive\s*22-01/i.test(normalized)) {
    add("BOD 22-01 対応");
  }

  if (/bod\s*26-04|binding operational directive\s*26-04/i.test(normalized)) {
    add("BOD 26-04 対応");
  }

  if (/cloud services?|cloud-based|cloud service provider|saas|iaas|paas/i.test(normalized)) {
    add("クラウドサービス向け対応");
  }
  ...
  if (hasVendorInstruction || hasApplicableGuidance) {
    add("ベンダ案内に従う");
  }

  if (/evaluate each asset|asset'?s .*exposure|asset exposure|asset inventory|identify affected assets/i.test(normalized)) {
    add("資産ごとの影響確認");
  }
...
function summarizeRequiredActionText(normalized, categories) {
  ...
  if (has("BOD 22-01 対応")) parts.push("BOD 22-01 の該当ガイダンス対応");
  if (has("BOD 26-04 対応")) parts.push("BOD 26-04 の該当ガイダンス対応");
  if (has("クラウドサービス向け対応")) parts.push("クラウドサービス向け対応");
  if (has("フォレンジックトリアージ")) parts.push("フォレンジックトリアージ要件の確認");
  if (has("インターネット露出評価")) parts.push("インターネット露出の評価");
  if (has("資産ごとの影響確認")) parts.push("資産ごとの影響確認");
  ...
  if (has("ベンダ案内に従う") && parts.length === 0) parts.push("ベンダ案内に従った対応");
