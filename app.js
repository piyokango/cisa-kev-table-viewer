...
async function fetchCatalog() {
  // GitHub Pages 上に配置した kev.json を取得します。
  // CISA 公式URLをブラウザから直接 fetch すると CORS により失敗する場合があるため、
  // GitHub Actions が取得して Pages に同梱した JSON を読む方式にしています。
  const primary = await fetchJson(KEV_JSON_URL).catch(async () => fetchJson(GITHUB_FALLBACK_URL));
